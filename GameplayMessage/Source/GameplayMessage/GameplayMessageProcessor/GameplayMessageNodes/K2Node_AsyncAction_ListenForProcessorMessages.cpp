...
#include "K2Node_AsyncAction_ListenForProcessorMessages.h"
...
#include "GameplayMessageRuntime/AsyncAction_ListenForProcessorMessages.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(K2Node_AsyncAction_ListenForProcessorMessages)

class UEdGraph;

#define LOCTEXT_NAMESPACE "K2Node"

namespace UK2Node_AsyncAction_ListenForProcessorMessagesHelper
{
...
void UK2Node_AsyncAction_ListenForProcessorMessages::PostReconstructNode()
{
...
void UK2Node_AsyncAction_ListenForProcessorMessages::PinDefaultValueChanged(UEdGraphPin* ChangedPin)
{
...
void UK2Node_AsyncAction_ListenForProcessorMessages::GetPinHoverText(const UEdGraphPin& Pin, FString& HoverTextOut) const
{
	Super::GetPinHoverText(Pin, HoverTextOut);
	if (Pin.PinName == UK2Node_AsyncAction_ListenForProcessorMessagesHelper::PayloadPinName)
	{
		HoverTextOut = HoverTextOut + LOCTEXT("PayloadOutTooltip", "\n\nThe message structure that we received").ToString();
	}
}

void UK2Node_AsyncAction_ListenForProcessorMessages::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
	struct GetMenuActions_Utils
	{
		static void SetNodeFunc(UEdGraphNode* NewNode, bool /*bIsTemplateNode*/, TWeakObjectPtr<UFunction> FunctionPtr)
		{
			UK2Node_AsyncAction_ListenForProcessorMessages* AsyncTaskNode = CastChecked<UK2Node_AsyncAction_ListenForProcessorMessages>(NewNode);
	...
	ActionRegistrar.RegisterClassFactoryActions<UAsyncAction_ListenForProcessorMessages>(FBlueprintActionDatabaseRegistrar::FMakeFuncSpawnerDelegate::CreateLambda([NodeClass](const UFunction* FactoryFunc)->UBlueprintNodeSpawner*
	{
...
void UK2Node_AsyncAction_ListenForProcessorMessages::AllocateDefaultPins()
{
	...
	UEdGraphPin* DelegateProxyPin = FindPin(UK2Node_AsyncAction_ListenForProcessorMessagesHelper::DelegateProxyPinName);
	if (ensure(DelegateProxyPin))
	{
		DelegateProxyPin->bHidden = true;
	}

	CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Wildcard, UK2Node_AsyncAction_ListenForProcessorMessagesHelper::PayloadPinName);
}

bool UK2Node_AsyncAction_ListenForProcessorMessages::HandleDelegates(const TArray<FBaseAsyncTaskHelper::FOutputPinAndLocalVariable>& VariableOutputs, UEdGraphPin* ProxyObjectPin, UEdGraphPin*& InOutLastThenPin, UEdGraph* SourceGraph, FKismetCompilerContext& CompilerContext)
{
	bool bIsErrorFree = true;

	if (VariableOutputs.Num() != 3)
	{
		ensureMsgf(false, TEXT("UK2Node_AsyncAction_ListenForProcessorMessages::HandleDelegates - Variable output array not valid. Output delegates must only have the single proxy object output and than must have pin for payload."));
		return false;
	}
...
void UK2Node_AsyncAction_ListenForProcessorMessages::RefreshOutputPayloadType()
{
...
UEdGraphPin* UK2Node_AsyncAction_ListenForProcessorMessages::GetPayloadPin() const
{
	UEdGraphPin* Pin = FindPinChecked(UK2Node_AsyncAction_ListenForProcessorMessagesHelper::PayloadPinName);
	check(Pin->Direction == EGPD_Output);
	return Pin;
}

UEdGraphPin* UK2Node_AsyncAction_ListenForProcessorMessages::GetPayloadTypePin() const
{
	UEdGraphPin* Pin = FindPinChecked(UK2Node_AsyncAction_ListenForProcessorMessagesHelper::PayloadTypePinName);
	check(Pin->Direction == EGPD_Input);
	return Pin;
}

UEdGraphPin* UK2Node_AsyncAction_ListenForProcessorMessages::GetOutputChannelPin() const
{
	UEdGraphPin* Pin = FindPinChecked(UK2Node_AsyncAction_ListenForProcessorMessagesHelper::ActualChannelPinName);
	check(Pin->Direction == EGPD_Output);
	return Pin;
}
